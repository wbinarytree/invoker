---
title: Yasha
kind: item
patch: 7.41d
card:
  entity: yasha
  sentences:
  - text: Yasha is an artifact item costing 2100 gold that grants 16 Agility, 15 Attack
      Speed, and 10% Movement Speed.
    marks:
    - gamefile:items/item_yasha#cost
    - gamefile:items/item_yasha#attribs
  - text: Its build formula is Blade of Alacrity (1000 gold), Band of Elvenskin (450
      gold), and Recipe (650 gold).
    marks:
    - gamefile:items/item_yasha#components
  - text: It builds into Manta Style, Sange and Yasha, Trident, and Yasha and Kaya.
    marks:
    - gamefile:items/item_yasha#components
  - text: Yasha is passive.
    marks:
    - gamefile:items/item_yasha#mechanics
  - text: Yasha is regarded as the swiftest weapon ever created.
    marks:
    - loc:DOTA_Tooltip_ability_item_yasha_Lore
---

# Yasha

Yasha is an artifact item that grants Agility, Attack Speed, and Movement Speed. [gamefile:items/item_yasha#cost] [gamefile:items/item_yasha#attribs]

## Stats

| Attribute | Value |
|---|---:|
| Cost | 2100 gold |

[gamefile:items/item_yasha#cost]

| Stat | Value |
|---|---:|
| AGILITY | 16 |
| ATTACK SPEED | 15 |
| MOVEMENT SPEED | 10% |

[gamefile:items/item_yasha#attribs]

## Components

| Component | Gold cost |
|---|---:|
| Blade of Alacrity | 1000 gold |
| Band of Elvenskin | 450 gold |
| Recipe | 650 gold |

**Builds into:**

| Item | Gold cost |
|---|---:|
| Manta Style | 4650 gold |
| Sange and Yasha | 4200 gold |
| Trident | 6301 gold |
| Yasha and Kaya | 4200 gold |

[gamefile:items/item_yasha#components]

## Mechanics

Yasha is passive. [gamefile:items/item_yasha#mechanics]

*Yasha is regarded as the swiftest weapon ever created.* [loc:DOTA_Tooltip_ability_item_yasha_Lore]