---
title: Fairy's Trinket
kind: item
patch: 7.41d
card:
  entity: mysterious_hat
  sentences:
  - text: Fairy's Trinket is a 0 gold item that provides 50 Health, 5% Manacost Reduction,
      and 5 Spell Amp.
    marks:
    - gamefile:items/item_mysterious_hat#cost
    - gamefile:items/item_mysterious_hat#attribs
  - text: Its build formula is Fairy's Trinket (0 gold).
    marks:
    - gamefile:items/item_mysterious_hat#cost
  - text: Its behavior is Passive.
    marks:
    - gamefile:items/item_mysterious_hat#mechanics
  - text: Its cooldown is 8.0.
    marks:
    - gamefile:items/item_mysterious_hat#mechanics
  - text: “A small token imbued with the fortune of the fae in recognition of an intriguing
      display of mortal kindness.”
    marks:
    - loc:DOTA_Tooltip_Ability_item_mysterious_hat_Lore
---

# Fairy's Trinket

Fairy's Trinket is an item that provides Health, Manacost Reduction, and Spell Amp. [gamefile:items/item_mysterious_hat#cost] [gamefile:items/item_mysterious_hat#attribs]

## Stats

| Stat | Value |
|---|---:|
| Health | 50 |
| Manacost Reduction | 5% |
| Spell Amp | 5 |

[gamefile:items/item_mysterious_hat#attribs]

## Components

| Item | Gold cost |
|---|---:|
| Fairy's Trinket | 0 gold |

[gamefile:items/item_mysterious_hat#cost]

## Mechanics

| Property | Value |
|---|---:|
| Behavior | Passive |
| Cooldown | 8.0 |

[gamefile:items/item_mysterious_hat#mechanics]

*A small token imbued with the fortune of the fae in recognition of an intriguing display of mortal kindness.* [loc:DOTA_Tooltip_Ability_item_mysterious_hat_Lore]