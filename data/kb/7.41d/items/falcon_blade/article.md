---
title: Falcon Blade
kind: item
patch: 7.41d
card:
  entity: falcon_blade
  sentences:
  - text: Falcon Blade is a rare item costing 1125 gold that provides 14 Damage, 200
      Health, and 1.8 Mana Regeneration.
    marks:
    - gamefile:items/item_falcon_blade#cost
    - gamefile:items/item_falcon_blade#attribs
  - text: Falcon Blade is built from Blades of Attack (450 gold), Fluffy Hat (250
      gold), Sage's Mask (175 gold), and a Recipe (250 gold).
    marks:
    - gamefile:items/item_falcon_blade#components
  - text: Falcon Blade has passive behavior.
    marks:
    - gamefile:items/item_falcon_blade#mechanics
  - text: An enchanted blade that long ago raised a hopeless urchin from pauper to
      king.
    marks:
    - loc:DOTA_Tooltip_ability_item_falcon_blade_Lore
---

# Falcon Blade

Falcon Blade is a rare item that provides Damage, Health, and Mana Regeneration. [gamefile:items/item_falcon_blade#cost] [gamefile:items/item_falcon_blade#attribs]

## Stats

| Stat | Value |
|---|---:|
| Damage | 14 |
| Health | 200 |
| Mana Regeneration | 1.8 |

[gamefile:items/item_falcon_blade#attribs]

## Components

| Component | Gold cost |
|---|---:|
| Blades of Attack | 450 gold |
| Fluffy Hat | 250 gold |
| Sage's Mask | 175 gold |
| Recipe | 250 gold |
| Falcon Blade | 1125 gold |

[gamefile:items/item_falcon_blade#components] [gamefile:items/item_falcon_blade#cost]

## Mechanics

Falcon Blade has passive behavior. [gamefile:items/item_falcon_blade#mechanics]

*An enchanted blade that long ago raised a hopeless urchin from pauper to king.* [loc:DOTA_Tooltip_ability_item_falcon_blade_Lore]