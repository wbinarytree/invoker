---
title: Gloves of Haste
kind: item
patch: 7.41d
card:
  entity: gloves
  sentences:
  - text: Gloves of Haste is a 450-gold component item that grants 20 Attack Speed.
    marks:
    - gamefile:items/item_gloves#cost
    - gamefile:items/item_gloves#attribs
  - text: Its behavior is passive.
    marks:
    - gamefile:items/item_gloves#mechanics
  - text: Its build formula is Gloves of Haste (450 gold).
    marks:
    - gamefile:items/item_gloves#cost
  - text: It builds into Armlet of Mordiggian, Hand of Midas, Maelstrom, and Power
      Treads.
    marks:
    - gamefile:items/item_gloves#components
  - text: A pair of magical gloves that seems to render weapons weightless.
    marks:
    - loc:DOTA_Tooltip_ability_item_gloves_Lore
---

# Gloves of Haste

Gloves of Haste is a component item that grants Attack Speed. [gamefile:items/item_gloves#cost] [gamefile:items/item_gloves#attribs]

## Stats

| Stat | Value |
|---|---:|
| Attack Speed | 20 |

[gamefile:items/item_gloves#attribs]

Its behavior is passive. [gamefile:items/item_gloves#mechanics]

## Components

| Item | Cost |
|---|---:|
| Gloves of Haste | 450 gold |

[gamefile:items/item_gloves#cost]

Builds into:

| Item | Cost |
|---|---:|
| Armlet of Mordiggian | 2500 gold |
| Hand of Midas | 2200 gold |
| Maelstrom | 2950 gold |
| Power Treads | 1400 gold |

[gamefile:items/item_gloves#components]

*A pair of magical gloves that seems to render weapons weightless.* [loc:DOTA_Tooltip_ability_item_gloves_Lore]