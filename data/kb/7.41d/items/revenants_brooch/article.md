---
title: Revenant's Brooch
kind: item
patch: 7.41d
card:
  entity: revenants_brooch
  sentences:
  - text: Revenant's Brooch is a rare 3300-gold item that grants 35 damage, 30% critical
      chance, 80% critical multiplier, and 15% spell lifesteal.
    marks:
    - gamefile:items/item_revenants_brooch#cost
    - gamefile:items/item_revenants_brooch#attribs
  - text: Its passive Phantom Critical gives each attack a chance to deal additional
      bonus magic damage based on the attack’s damage.
    marks:
    - gamefile:items/item_revenants_brooch#mechanics
    - loc:DOTA_Tooltip_Ability_item_revenants_brooch_Description
  - text: It is built from Crystalys (2000 gold) and Voodoo Mask (650 gold), with
      a 650-gold recipe.
    marks:
    - gamefile:items/item_revenants_brooch#components
---

# Revenant's Brooch

Revenant's Brooch is a rare item that grants damage, critical chance, critical multiplier, and spell lifesteal. [gamefile:items/item_revenants_brooch#cost] [gamefile:items/item_revenants_brooch#attribs]

## Cost

| Item | Gold cost |
|---|---:|
| Revenant's Brooch | 3300 |

[gamefile:items/item_revenants_brooch#cost]

## Components

| Component | Gold cost |
|---|---:|
| Crystalys | 2000 |
| Voodoo Mask | 650 |
| Recipe | 650 |

[gamefile:items/item_revenants_brooch#components]

## Stats

| Stat | Value |
|---|---:|
| Damage | 35 |
| Critical chance | 30% |
| Critical multiplier | 80% |
| Spell lifesteal | 15% |

[gamefile:items/item_revenants_brooch#attribs]

## Mechanics

Phantom Critical is passive. Each attack has a chance to deal additional bonus magic damage based on the attack’s damage. [gamefile:items/item_revenants_brooch#mechanics] [loc:DOTA_Tooltip_Ability_item_revenants_brooch_Description]

*The cursed brooch of a fallen guardian who stalks forever between the veil of life and death.* [loc:DOTA_Tooltip_ability_item_revenants_brooch_Lore]